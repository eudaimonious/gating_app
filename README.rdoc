...
The Gating API is an isolated service that enables any of our applications to check what capabilities should be enabled or disabled for a given application. This allows us to activate capabilities for clients individually, do limited rollouts of new capabilities, and charge clients for selected capabilities. You can visit the app at link:gating.herokuapp.com.
...
* All Capabilities have:
  ...
  * (COMPLETE) A default value (i.e. false)
  * (COMPLETE) A type of Boolean, Numeric or String
  * (PARTIAL - Field exists, need to set based on logged in user) A User who is the Owner
* All Capabilities may have:
  * (PARTIAL - Change from datetime to date) An expiration date
  * (PARTIAL - Explain to users that notes can be added after capability is created.) A note (i.e. “When enabled, allow access to the new search API”)
...
* (PARTIAL - Need organization to reset when a user is promoted to Admin. Try adding org to the promotion form. You could change the value via the template. Also check whether adding to the form make the helper method in the controller work. Is it possible to do this in the models? What's best practice?) Users are a part of an Organization; Admins are not a part of any Organization
